"""ledger v0 — a server that works.

Four tools over the rights catalogue. This is deliberately the naive version:
it is correct, it is tested, and it will fall over the moment a model asks about
a popular title. That failure is the subject of chapters 8 and 10, and the code
here is left honest so the measurement is real.
"""
...
mcp = FastMCP(
    name="ledger",
    instructions=(
        "Rights catalogue for a media licensing operation. Titles carry licences; "
        "a licence grants named rights in one territory for a window of time, "
        "under an agreement with a licensee."
    ),
)
