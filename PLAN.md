...
└── .github/workflows/ci.yml    # run every code sample, lint every chapter link
```
...
| Phase | Output | Gate |
|---|---|---|
| 0 | Scaffolding: repo layout, chapter template, `build.py`, CI, README | A dummy chapter builds and opens |
...
1. **Spine domain.** Media rights is proposed above because it produces the right problems
   honestly. A generic alternative (a support-desk / ticketing estate) is less interesting
   but less likely to read as domain-specific. Decide before Phase 1.
2. **Backing store.** SQLite keeps the reader's setup to zero; Postgres makes the
   multi-tenancy and concurrency chapters truthful. Likely: SQLite by default, Postgres via
   docker-compose from ch. 11.
3. **Distribution.** Public repo as-is, or repo + a built site, or a gated academy edition
   with the labs held back. Affects nothing before Phase 6.
